...
#当程序运行到for的时候其实它的实际操作是这样的
""" 
第一次循环的时候：
n=3 
g = (add(n, i) for i in g)
#创建一个名为g的生成器推导式的右边并没有运行，当被第二次产生的生成器推导式g调用时，推导式右侧的g指向test()，同时n的值是11

n = 11 
g = (add(n, i) for i in g)
#创建另一个名为g的生成器推导式的右边并没有运行，当print(list(g))的时候，
# 推导式右侧的g指向的是for循环第一次创建的生成器推导式的g。同时n的值是11，因为for循环已经结束了。

#生成器函数只在3种情况下会运行，1.next(g) 2.for循环g 3.list(g) 这三种方法底层都是调用的next方法。
"""
print(list(g))  #此时list调用函数了，但是这个时候g指向的是for循环的第二次的g的生成器推导式
